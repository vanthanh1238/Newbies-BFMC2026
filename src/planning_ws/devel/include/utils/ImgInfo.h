  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::utils::ImgInfo_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.colorImg.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.colorImg[i]);
    }
    if (v.colorImg.empty() || true)
      s << "]";
    ...
    for (size_t i = 0; i < v.depImg.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint32_t>::stream(s, true ? std::string() : indent + "    ", v.depImg[i]);
    }
    if (v.depImg.empty() || true)
      s << "]";
  }
